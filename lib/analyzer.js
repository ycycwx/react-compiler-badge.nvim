...
function collectNoMemoNeededLines(events) {
    const lines = new Set();
    ...
    for (const event of events) {
        ...
        const memoSlots = event.memoSlots || 0;
        const memoBlocks = event.memoBlocks || 0;
        const memoValues = event.memoValues || 0;
        if (memoSlots === 0 && memoBlocks === 0 && memoValues === 0) {
            lines.add(line);
        }
    }
...
function analyze(sourceCode) {
    ...
    const noMemoNeededLines = collectNoMemoNeededLines(compilerEvents);
    const failed = uniqueSorted(candidateLines)
        .filter(line => !optimizedSet.has(line))
        .filter(line => !noMemoNeededLines.has(line))
        .map(line => {
